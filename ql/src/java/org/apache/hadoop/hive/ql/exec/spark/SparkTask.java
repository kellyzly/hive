  ...
  public int execute(DriverContext driverContext) {
    ...
    try {
      ...
      SparkWork sparkWork = getWork();
        broadcastSmallTables(sparkWork);
      ...
      if (rc == 0) {
        ...
        //  sparkSession.broadcastVariable();
    ...
    private void broadcastSmallTables(SparkWork sparkWork) {

        List<BaseWork> allBaseWorks  = sparkWork.getAllWork();
        for( BaseWork baseWork: allBaseWorks) {
            Set<Operator<?>> mapJoins = SparkMapJoinResolver.getOp(baseWork, MapJoinOperator.class);
            if( mapJoins.size() >0){
                for(Operator mapJoin: mapJoins) {
                    MapredLocalWork mapredLocalWork = (MapredLocalWork) baseWork.getMapRedLocalWork();
                    Path baseDir = mapredLocalWork.getTmpPath();
                    if (baseDir != null) {
                        console.printInfo("small table is in " + mapredLocalWork.getTmpPath());
                         loadHashTable((MapJoinOperator)mapJoin);
...
//
    private void loadHashTable(MapJoinOperator mapJoin){
        JobConf jobConf = new JobConf(conf);

        final ExecMapperContext mapContext = new ExecMapperContext(jobConf);
        final MapredContext mrContext = MapredContext.get();
        try {
            mapJoin.initializeOpFromConf(conf);
            mapJoin.loadHashTableToMemory(mapContext, mrContext);
        } catch (HiveException e) {
           console.printError("SparkTask.loadHashTable throws out err ",e.getLocalizedMessage());
        }

    }


//    private void load(Path baseDir, MapredLocalWork localWork) {
